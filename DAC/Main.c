...
static uint32_t freq_table[2] = {5000,10000,20000};// levels of TIM3_ARR.
...
int main(void){
	...
	ADC_start();
	while(1){
		vol = (uint32_t)(ADC1->DR * (33/65536)); // change to 3.3V
//		DR_value = ADC1->DR;
		CR_value = RCC->CR;
		CFGR_value = RCC->CFGR;
		ODR_value = GPIOB->ODR;
...
void PinConf(){
	...
	GPIOC->ODR &= ~(1<<13);
	//DAC
	GPIOB->CRL = 0x33333333; // output pp 50MHz.
	//EXTI_A10, A11:
	GPIOA->CRH &= ~(0xFF<<8);
	GPIOA->CRH |= 1<<15 | 1<<11;
 }
...
void ADC_conf(void){
	...
	ADC1->CR2 |= 1<<11; // alignADC1->CR2 |= 0x02; // continuos
...
void SquareTable_Init(void) {
    // t?o table lookup cho xung vuông.
    for (int i = 0; i < TABLE_SIZE / 2; i++) {
				square_table[i] = 0x0;
    }
    for (int i = TABLE_SIZE / 2; i < TABLE_SIZE; i++) {
